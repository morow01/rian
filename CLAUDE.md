...
### Copy/Move Task Picker Fixes (v6.8.17–v6.8.22)
...
**Weekend collapse — tried v6.8.17 through v6.8.21, removed in v6.8.22.** A collapsible "› Weekend" section (matching Timesheet's pattern) went through several fix rounds — a day-ordering bug (rewritten to match by day name instead of loop index), a missing CSS transition (toggle snapped instead of animating), and a "weird horizontal line" (missing `-webkit-tap-highlight-color` + the divider being inset instead of full-bleed within the picker's padded container). After all of that the user asked to just remove it. `buildCopyPickerDaysHtml()` is back to a plain flat loop over all 7 days, no divider, no collapse, no `toggleCopyPickerWeekend()`. **Don't re-add this without being asked** — the picker's small, low-stakes day list didn't need decluttering the way Timesheet's real task list does, and it cost more churn than it was worth. One remnant was kept: `.weekend-divider`'s shared base CSS rule now has `-webkit-tap-highlight-color: transparent` (added while chasing the tap-highlight theory) — left in place since it's a harmless general improvement Timesheet's own divider benefits from too, not something specific to the removed picker feature.
