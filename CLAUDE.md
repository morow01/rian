...
**Logo v2 — patch-panel F (v6.8.81):** the F + tick logo was replaced by a 3×3 dot grid where the F is lit (stem sky `#5B9BFF`, arms amber `#F5A623`, the other three ports dimmed to 28% opacity) on navy `#0F1C2E`. Source of truth = geometry in `images/fieldlog-{glyph,icon,favicon}.svg` (pitch 26, dot radius 10 in a 100-box; favicon uses r=11.5 so the F survives at 16px). PNGs (`icon-192/512.png`, all `mipmap-*/ic_launcher{,_round,_foreground}.png`) were rasterised from that geometry with a browser canvas (v6.8.84 padding pass: grid extent 46% of the tile / PWA icons, 40% on the round icon, 34% of the adaptive-icon foreground — the launcher mask crops the 108dp foreground to ~72dp, so 34% of 108dp ≈ 51% of the visible icon); adaptive background `ic_launcher_background` is now `#0F1C2E`. Design options live in untracked `mockups/Logo Options*.html`. The APK launcher icon only changes after an APK rebuild + reinstall.
...
- All `rian_*` localStorage/sessionStorage keys (renaming would orphan every user's saved settings and offline data), the `rian-export-v1` / `rian-backup-v1` / `rian-backup-index-v1` file formats (old exports must still import), the `rian-pending-note` cache (shared by `sw.js` ↔ `app.html`), the `@rian-pwa` calendar-UID suffix (re-imported ICS events dedupe on it), the `data-rian-note-merge` marker (already saved inside users' notes), the `offline@rian.local` Local-Mode placeholder email and the `rianfieldlog@gmail.com` test account.
- **Stripped in v6.8.85 (safe internal names):** console prefixes `[Rian …]` → `[FieldLog …]` (diagnostic recipes here now quote `[FieldLog]`), `showRianDialog` → `showAppDialog`, `#rian-dialog*` → `#app-dialog*`, `_rianCheckForUpdate/_rianDoRefresh/_rianShowUpdateToast/_rianUpdateAvailable/_rianExitConfirmed` → `_app…`, `rianUpdatePulse` → `appUpdatePulse`, `_rian_email/_rian_photoURL/_rian_displayName` → `_app_*`, the dev-bypass session keys `_rian_bypass_*` → `_app_bypass_*`, service-worker cache prefix `rian-v` → `fieldlog-v` (old caches are deleted on activate), notification tags, and every comment. Older notes below may still show the old names.
...
**Menu content restructure (v6.8.74):** the ☰ menu was one ~3-screen scroll (12 two-line destination rows + AI key + display switches + data tools). Now:
- **Destinations** are one-line rows (`_menuNavHtml()`, `.menu-row-compact`, pin buttons kept via top-level `_menuPinBtn()`). AI Assistant moved into this list; Notes Tag Manager + Task Templates moved under Settings → Manage.
- **Mobile / tablet (<1280px):** Settings are 4 collapsible groups (`_menuSettingsAccordions()` → Manage, Display, AI key, Data & backups incl. a "Danger zone" sub-heading). Open/closed lives in `state.menuAcc` (not persisted); `toggleMenuAcc(id)` flips the class in place — no re-render.
- **Desktop (`_isDesktop()`):** the drawer is navigation only + a "Settings" launcher row; Settings opens a centred dialog, `#settings-dlg` (`openSettingsDialog()` / `closeSettingsDialog()` / `settingsDlgCat(c)`), left rail Manage · Display · AI key · Data & backups · Danger zone. Closed by ESC (added to both ESC stacks), backdrop, X. Notes Tag Manager stays hidden on desktop (lives in the Notes sidebar).
...
- **Closed desktop drawer stole taps (v6.8.86).** `.menu-acc.open .menu-acc-in` used `visibility: visible`, which overrides the inherited `visibility:hidden` of the closed desktop drawer — so the open Navigation group's rows stayed hit-testable and a tap on e.g. the Monday day row opened Journal instead (reproduced on the Tab S8+ by dispatching a real touch via CDP and reading `elementFromPoint`). Now `visibility: inherit`. Any future descendant of `.theme-dropdown` must not force `visibility: visible`.
- **Desktop panels fill the viewport (v6.8.87).** `.desk-three-panel`, `.dnotes-wrap`, `.dj-wrap`, `.dco-wrap`, `.dfnd-wrap`, `.hist-desk-wrap` had hard-coded `calc(100vh - 90/110/180px)` heights that assumed a fixed header, leaving a grey band under the app on the Tab S8+. `_fitDeskPanels()` now sets their height from their measured top edge to the bottom of the viewport (after every `_renderNow()`, `updateHeaderStatus()`, resize and orientation change); the CSS calc() values remain as the pre-JS fallback. `#app` padding-bottom is 0 at >=1280px (no bottom nav there). Add any new full-height desktop panel class to `_DESK_FIT_SEL`.
- **Week History shows the week-role pill (v6.8.88).** `_avWeekPillHtml(weekStart)` (next to `_avPill`) renders the same SD / SA / R / D / L pill the Timesheet week selector shows (from `_avWeekRole`; on-call weeks OC/50 are skipped because the On-Call tag already covers them). Used in the mobile week rows (`.hist-week-range`) and the desktop table (`.hdt-range`).
- **AL pill + On-call pill restyle (v6.8.89–90).** `_avWeekAlPillHtml(ws)` renders an `AL` pill (`.av-al`, title = day count) when any day in the week is `A/L`; it is appended after the SD/SA/… role pill in the Timesheet week selector (mobile + desktop) and inside `_avWeekPillHtml()` for Week History. `.week-status-pill.is-oncall` now uses `--pill-callout-*`, 7px radius, 12px sentence case — identical to the callout-count badge `.hist-badge.red`. On-call weeks are schedule code `OC` and carry no SD/SA (the data cycle is SD, SD, SA, then the on-call week), so they show the On-Call pill only. The mobile selector row wraps (`.page-week-selector-range{flex-wrap:wrap}`) so three pills push onto a second line instead of truncating the date.
...
- **Pinned top tabs are per layout (v6.8.78):** `getVisibleTabs()` / `toggleTopTabVisibility()` use `_visibleTabsKey()` — `rian_visible_tabs_desk` when `_isDesktop()`, otherwise the original `rian_visible_tabs` (mobile). Desktop reads the shared key as a fallback until its own list is first changed, so existing pins carry over once. Local to the device (not synced), like before.
...
**Menu account block + header logo (v6.8.70):** the ☰ menu's tall centred account section (avatar, "Signed in as", name, email, full-width Sign Out — ~195px) is now one compact row, `.menu-header-block` (~64px): avatar · name/email (ellipsised) · sign-out icon (`.menu-hdr-signout`) · close X. Sign-out goes through `confirmSignOut()` (a `showAppDialog` confirm) since the icon sits next to the close button. Local Mode keeps its two actions ("Connect Google" / "Exit Local Mode") in a second row (`.menu-local-actions`). (A small glyph logo was briefly shown before the sync status in the main header in v6.8.70–71 and removed again in v6.8.72 at the user's request — the main header carries no logo.)
...
When a note or task note has remote changes during fullscreen TipTap editing (`closeNoteFullscreen()`), the conflict dialog now offers a dedicated "Return to editor" option alongside "Keep cloud version" and "Save mine". Clicking "Return to editor", pressing ESC, or tapping the overlay backdrop immediately returns to the editor with all unsaved text edits and cursor position intact without closing or discarding. `showAppDialog()` updated to support `cancelValue` and `value` on `extraBtns`.
...
```
`[FieldLog] Write verified on server (N activities)` is the single most useful log line — it says what actually reached the cloud.
...
Added a completely SW-independent check in `init()`: `_appCheckForUpdate()` fetches `app.html` directly with `cache:'no-store'`, regexes out `const VERSION = '...'`, and compares against the running `VERSION`. Runs once ~4s after load and again on every `visibilitychange` foreground. On a mismatch, shows the same `showActionToast('Update ready (vX)', 'Refresh', ...)` UI as the v6.8.8 controllerchange path (both funnel through `_appShowUpdateToast()` now, gated by one shared `_rianUpdateToastShown` flag so they can't double-prompt) — but the refresh action navigates to `location.pathname + '?_r=' + Date.now()` rather than a plain `location.reload()`, since a plain reload could hit whatever cache layer was the actual problem; a genuinely new URL can't be served from any cache keyed by exact URL.
...
Implementation: `_appUpdateAvailable` (top-level `var`, not nested in `init()`) holds the detected new-version string once found; the header template in `render()` reads it directly and conditionally renders the icon (`@keyframes appUpdatePulse` for the subtle pulse). `_appShowUpdateToast(newVersion)` (name kept for continuity even though it no longer shows a toast) sets the flag and calls `render()`. `_appDoRefresh()` — called from the icon's `onclick`, which executes in global scope — navigates to `location.pathname + '?_r=' + Date.now()`, same cache-busting approach as before.

All four functions (`_appUpdateAvailable`, `_appDoRefresh`, `_appShowUpdateToast`, `_appCheckForUpdate`) moved to top-level scope (near `showActionToast`, well before `render()` and `init()`) — they were previously nested inside `init()`, which worked for the wiring (`setTimeout`, event listeners) but meant `render()`'s header template couldn't read `_appUpdateAvailable` and the `onclick="_appDoRefresh()"` attribute couldn't resolve the function, since neither runs inside `init()`'s closure.
